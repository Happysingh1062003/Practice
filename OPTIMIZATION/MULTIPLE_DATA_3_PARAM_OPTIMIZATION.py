...
import pandas as pd
import numpy as np
from STRATEGY_3_PARAM import entry_signals,exit_signals

def backtest_and_analyze(data,symbol_name, entry_signals_func, exit_signals_func, parameter_1, parameter_2,parameter_3,name_param_1,name_param_2,name_param_3, freq, fees):
    # Initialize storage for metrics
    metrics = {
        'SYMBOL' :[],
        f'{name_param_1}_Window': [],
        f'{name_param_2}_Window': [], 
        f'{name_param_3}_Window': [],
        'Returns': [],
        'Sharpe_Ratio': [],
        'Profit_Factor': [],
        'Win_Rate' : [],
    ...
    for param_1 in parameter_1:
        for param_2 in parameter_2: 
            for param_3 in parameter_3:
                # Generate entry and exit signals
                entry_signals = entry_signals_func(data, param_1, param_2,param_3) 
                exit_signals = exit_signals_func(data, param_1, param_2,param_3) 
                ...
                metrics[f'{name_param_2}_Window'].append(param_2)
                metrics[f'{name_param_3}_Window'].append(param_3)  
    ...
    result_df['Parameter'] = f'{name_param_1}_' + result_df[f'{name_param_1}_Window'].astype(str) + f'-{name_param_2}_' + result_df[f'{name_param_2}_Window'].astype(str) + f'-{name_param_3}_' + result_df[f'{name_param_3}_Window'].astype(str) 
    return result_df
...
if __name__ == "__main__":
    ...
    '''==========================================================================================================================================================================================='''
    # PUT THE PARAMETER 
    std_dev=np.arange(2,6)
    bb_window = np.arange(10, 30, 10)
    rsi_window = np.arange(10, 16, 2)
    ...
    for data , name in zip(in_sample_symbol,symbol_name):  #CHANGE THE IN_SAMPLE_SYMBOL TO SYMBOL IF REQUIRED
        results = backtest_and_analyze(
            ...
            parameter_1=bb_window,
            parameter_2=rsi_window,
            parameter_3=std_dev,
            name_param_1='bb', #Change the Name of Parameter
            name_param_2='rsi', #Change the Name of Parameter
            name_param_3='stddev', #Change the Name of Parameter
            freq=timeframe,
            fees=0.001
        )
...
all_results.to_csv('3_param.csv', index=False)
