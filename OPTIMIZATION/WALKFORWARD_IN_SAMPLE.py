...
lowerband=BB.run(data_in_sample[stage],20,3,3).lowerband.to_numpy()
rsivalue=RSI.run(data_in_sample[stage],14).real.to_numpy()
upperband=BB.run(data_in_sample[stage],20,3,3).upperband.to_numpy()

# print(help(BB.run))

#LOGIC
entry=  (data_in_sample[stage]< lowerband) & (rsivalue < 30) 
exits = (data_in_sample[stage] > upperband) & (rsivalue > 70)
...
#BACKTEST ENGINE
strategy = vbt.Portfolio.from_signals(data_in_sample[stage], entry, exits,freq="30min",fees=0.001,size=1)
