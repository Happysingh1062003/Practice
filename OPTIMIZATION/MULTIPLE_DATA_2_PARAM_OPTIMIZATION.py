...
import pandas as pd
import numpy as np
from STRATEGY_2_PARAM import entry_signals,exit_signals

def backtest_and_analyze(data,symbol_name, entry_signals_func, exit_signals_func, parameter_1, parameter_2,name_param_1,name_param_2, freq, fees):
    ...
    for param_1 in parameter_1: #here
        for param_2 in parameter_2: #here
            # Generate entry and exit signals
            entry_signals = entry_signals_func(data, param_1, param_2) 
            exit_signals = exit_signals_func(data, param_1, param_2) 
    ...
    result_df['Parameter'] = f'{name_param_1}_' + result_df[f'{name_param_1}_Window'].astype(str) + f'-{name_param_2}_' + result_df[f'{name_param_2}_Window'].astype(str)
    return result_df
...
if __name__ == "__main__":
    ...
    # PARAMETER 
    bb_window = np.arange(10, 50,10)
    rsi_window = np.arange(10,22,2)
...
all_results.to_csv('2_param.csv', index=False)
